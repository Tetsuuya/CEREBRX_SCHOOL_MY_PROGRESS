        ...
        <div class="row">          
            <div class="col-md-12">              
                ...
                <div class="box box-primary" id="sublist"> 
                    ...
                    <div class="box-body">
                        <form id='form1' action="<?php echo site_url('registrar/report/grade_sheet') ?>"  method="post" accept-charset="utf-8">
                            <div class="box-body">
                                  ...
                                  <table class="table table-hover table-striped table-bordered xyz example">
                                    ...
                                    <tbody>
                                        <?php if (empty($studentlist_male)) {
                                            ?>
                                            <tr>
                                                <td colspan="<?php echo $enableStrand ? $total_quarter + 2 : 5; ?>" class="text-danger text-center"><?php echo $this->lang->line('no_record_found'); ?></td>
														...
														for( $qtr=0;$qtr<$total_quarter;$qtr++ ) {
															...
															if( empty( $getStudentSubjectSemesterDrop)){
																...
																if( $grade_per_quarter == null || $grade_per_quarter == 0 ){
																	...
																	$final_grade = $final_grade + $grade_per_quarter;
																	$text_danger = ($grade_per_quarter < 75) ? 'class="text-danger"' : '';
																	?>
																	<th <?php echo $text_danger; ?>><?php echo $grade_per_quarter;?></th>
																	<?php
																}
															} else {
																$not_available = true;
...
<script type="text/javascript"> 
    ...
    $(document).ready(function () {
        ...
        var semester_id_post = '<?php echo $semester_id; ?>';
        populateSection(section_id_post, class_id_post, subject_id_post,session_id_post, semester_id_post);
        function populateSection(section_id_post, class_id_post, subject_id_post,session_id_post, semester_id_post) {
            $('#section_id').html("");
            ...
            $.ajax({
                type: "GET",
                url: base_url + "registrar/subject/getSubjctByClass",
				data: {'class_id': class_id_post, 'semester_id' : semester_id_post, 'session_id' : session_id_post },
                dataType: "json",
                success: function (data) {
                    $.each(data, function (i, obj)
                    {
        ...
        $(document).on('change', '#class_id', function (e) {
            ...
            var semester_id = $('#select_strand').val();
            var base_url = '<?php echo base_url() ?>';
            var div_data = '<option value=""><?php echo $this->lang->line('select'); ?></option>';
            $.ajax({
                type: "GET",
            ...
            $.ajax({
                type: "GET",
                url: base_url + "registrar/subject/getSubjctByClass",
                data: {'class_id': class_id, 'semester_id' : semester_id, 'session_id' : session_id },
                dataType: "json",
                success: function (data) {
                    $.each(data, function (i, obj)
                    {
		...
		$(document).on('change', '#session_id', function (e) {
			...
			var subject_id_post = $('#subject_id').val();
			var semester_id_post = $('#select_strand').val();
			populateSection(section_id_post, class_id_post, subject_id_post, session_id_post, semester_id_post );
		});
		$(document).on('change', '#select_strand', function (e) {
			$('#subject_id').html("");
			var section_id_post = $('#section_id').val();
			var class_id_post = $('#class_id').val();
			var session_id_post = $('#session_id').val();
