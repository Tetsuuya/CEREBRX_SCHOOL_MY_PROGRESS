...
class Report extends CI_Controller {
    ...
    function pdfByInvoiceNo() {
        ...
        $data['student_due_fee'] = $student_due_fee;
        $html = $this->load->view('teacher/reports/pdfinvoiceno', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function pdfDepositeFeeByStudent($id) {
        ...
        $html = $this->load->view('teacher/reports/pdfStudentDeposite', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function pdfStudentListByText() {
        ...
        $html = $this->load->view('teacher/reports/pdfStudentListByText', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function pdfStudentListByClassSection() {
        ...
        $data['resultlist'] = $resultlist;
        $html = $this->load->view('teacher/reports/pdfStudentListByClassSection', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function pdfStudentListDifferentCriteria() {
        ...
        $data['resultlist'] = $resultlist;
        $html = $this->load->view('teacher/reports/pdfStudentListDifferentCriteria', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function pdfStudentListByClass() {
        ...
        $resultlist = $this->student_model->searchByClassSection($class_id, $section_id);
        $data['resultlist'] = $resultlist;
        $html = $this->load->view('teacher/reports/pdfStudentListByClass', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
    ...
    function transactionSearch() {
        ...
        $data['settinglist'] = $setting_result;
        $html = $this->load->view('teacher/reports/transactionSearch', $data, true);
        $pdfFilePath = $this->time . ".pdf";
        $this->load->library('m_pdf');
        $this->m_pdf->pdf->WriteHTML($html);
        $this->m_pdf->pdf->Output($pdfFilePath, "D");
	...
	function student_ranking(){
       ...
       /*  $teacher_id = $student_session_data['teacher_id']; 
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('teacher/reports/student_ranking', $data);
	...
	function honor_students(){
		$this->session->set_userdata('top_menu', 'Reports');
		$this->session->set_userdata('sub_menu', 'report/honor_students');
		$data = array();
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/reports/honor_student', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $resultlist = array();
            $class = $this->input->post('class_id');
	...
	function report_card(){
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/reports/report_card', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $resultlist = array();
            $class = $this->input->post('class_id');
	...
	function master_sheet(){
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/reports/master_sheet', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $resultlist = array();
            $class = $this->input->post('class_id');
	...
	function grade_sheet(){
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/reports/grade_sheet', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $resultlist = array();
            $class = $this->input->post('class_id');
            ...
            if( isset($action) && $action == 'print_grade_sheet'){  
                $this->print_grade_sheet( $class, $section, $subject, $semester_id );
            }
        }  
	...
	public function show_report_card(){
		...
		if( $student_id ){
			...
			$size = !empty($getFormSettings['size'])?$getFormSettings['size']:'LEGAL';
			...
			$default_file = !empty($getFormSettings['default_file'])?$getFormSettings['default_file']:'Form138Template';
			...
			//$teacher_advisory =  $this->classsection_model->get_teacher_advisory( $class_id,  $section_id );
			...
			if( count( $student_print ) > 1 || count( $student_id ) > 1){
				for( $x=0; $x<count( $student_print );$x++){
                    /* $student_attendance_present_array = $this->stuattendencecustom_model->get_attendance( $student_id[$x], 'Present', $	 );
                    $student_attendance_present = $student_attendance_present_array['attendance'];
                    if( $student_attendance_present ){
                        $student_attendance_present = unserialize($student_attendance_present);
                    }

                    $student_attendance_late_array = $this->stuattendencecustom_model->get_attendance( $student_id[$x], 'Late', $session_id );
                    $student_attendance_late = $student_attendance_late_array['attendance'];
                    if( $student_attendance_late ){
                        $student_attendance_late = unserialize($student_attendance_late);
                    }

                    $student_attendance_absent_array = $this->stuattendencecustom_model->get_attendance( $student_id[$x], 'Absent', $session_id );
                    $student_attendance_absent = $student_attendance_absent_array['attendance'];
                    if( $student_attendance_absent ){
                        $student_attendance_absent = unserialize($student_attendance_absent);
                    }
                    $data['student_attendance_present'] =  $student_attendance_present;
                    $data['student_attendance_late'] =  $student_attendance_late;
                    $data['student_attendance_absent'] =  $student_attendance_absent; */
					// $this->m_pdf->pdf->AddPage($orientation );
					$this->m_pdf->pdf->AddPage($orientation,$size,'5','5','5','5','5','5','5');
                ...
                $student_attendance_late_array = $this->stuattendencecustom_model->get_attendance( $student_id, 'Late', $session_id );
                $student_attendance_late = $student_attendance_late_array['attendance'];
                if( $student_attendance_late ){
                    $student_attendance_late = unserialize($student_attendance_late);
                }
                ...
                $data['student_attendance_late'] =  $student_attendance_late;
                $data['student_attendance_absent'] =  $student_attendance_absent; */
				// $this->m_pdf->pdf->AddPage($orientation,$size,'5','5','5','5','5','5','5','5');
                $this->m_pdf->pdf->AddPage($orientation,$size,'5','5','5','5','5','5','5');
			...
			redirect('teacher/report/report_card');
		}
	}
		...
		if( $data ){
			...
			$data = array_merge( $data, $get_layout_conduct );
			  ...
			  /*   $this->mpdf->AddPage('L', // L - landscape, P - portrait
					...
					12); // margin footer */
             // printx($filename);
		   $html = $this->load->view('template/'.$folder.'/'.$filename,$data, true ); 
		   /* $pdfFilePath = "Form138".$data['fullname'].".pdf";
		   $this->load->library('m_pdf');
	...
	public function get_layout_attendance( $data ){
		...
		ob_start(); 
			?>
			<tr>
			<td style="border: 1px solid;padding-left:"><small>Days of School</small></td>
			<?php 
				...
				while($month < $end){   
					...
					?> 
					<td style="border: 1px solid;" align="center" ><small><?php echo $month_count; ?></small></td>  
					<?php
				} 
				?>
			...
			<td style="border: 1px solid;"><small>Days Present</small></td>
				...
				while($month < $end){   
					...
					if( !empty( $student_attendance_present_serialize[$monthly_name_fd])){
	...
	public function get_layout_grade( $data ){
		...
		ob_start(); 
			...
			if( $semester ){
				...
				$list_of_subjects = $this->customsubject_model->unsetsubjectdrop( $list_of_subjects, $student_id, $semester );
				...
				if( $quarter == 'final'){
					foreach( $list_of_subjects as $listsubject => $subjects ){
						...
						if( $display_card == 'yes' ){
							...
							if( $align_right == 'yes' ){
								?>
								<td style="padding-left:20px;font-size:13px;"><?php echo $subjects['name'];?></td>
								<?php
							} else {
								?>
								<td style="padding-left:10px;font-size:13px;"><?php echo $subjects['name'];?></td>
								<?php
							}	
						}	
						$get_final_grade = 0;
								...
								if( $display_card == 'yes' ){
									if( $grade_per_quarter == null || $grade_per_quarter == 0  ){
										if( $if_gradeisnull == 'blank'){
											...
											?>
											<td style="font-size:13px;"><?php echo number_format((float)0, $decimal_grades, '.', '');?></td>
											<?php
										}
									} else {
										?>
										<td align="center" style="font-size:13px;"><?php echo $grade_per_quarter;?></td>
										<?php
									}
								}
						...
						if( $display_card == 'yes' ){
							if( $complete_grades ){
								?>
								<td align="center" style="font-size:13px;"><?php echo $get_final_grade_result;?></td>
								<td align="center"><small><?php echo $get_final_remarks;?></small></td>
								</tr>
								<?php
							} else {
								?>
					...
					foreach( $list_of_subjects as $listsubject => $subjects ){
						...
						if( $display_card == 'yes' ){ 
							if( $complete_grades ){
								?>
								<td align="center"  style="font-size:13px;"><?php echo $get_final_grade_result;?></td>	
								<td align="center"  style="font-size:13px;"><small><?php echo $get_final_remarks;?></small></td>				
								</tr>
								<?php
							} else {
								?>
				...
				$get_quarter_settings = array('1','2','3','4');	
				$get_quarter_settings = is_serialized( $get_quarter_settings )?unserialize($get_quarter_settings):$get_quarter_settings;
				$data['set_quarter'] = $get_quarter_settings; 
			}
							...
							<tr>
								...
								for($x=0;$x<count($set_quarter);$x++){
									...
									if( $get_quarter <= $quarter  ){
										$conduct_details = $this->conductimportbatchdetail_model->getAllDetailsConductByStudentPerQuarter( $conduct_id, $student_id, $class_id, $section_id, $get_quarter );
				...
				?>
					...
					if (!empty( $studentlist_male )) { 
						...
						foreach ($studentlist_male as  $student_value) { 
							...
							if( $quarter != "final"){
								echo '<td style= "border: 1px solid black; font-size: 12px;  padding-left: 6px;  padding-right: 6px;">';
								$student_conduct = '';
   ...
   public function print_grade_sheet( $class_id, $section_id, $subject_id, $semester_id=NULL ){  
        $setting_result = $this->setting_model->get();
		$session_id = $setting_result[0]['session_id'];
